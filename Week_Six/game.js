...
onload = function() {
    let RandomCounty = countriesAndCodes[Math.floor(Math.random()*countriesAndCodes.length)]
    let RandomCountryDisplay = RandomCounty.name
// TODO display the country's name in the randomCountryElement     
    randomCountryElement.innerHTML = RandomCountryDisplay
    countryCode.push(RandomCounty['alpha-2'])   
...
submitButton.addEventListener('click', function() {
    ...
    fetch(url)
        ...
        .then( actualCapital => {  // function         
            let actualCapitalCity = actualCapital[1][0]['capitalCity']
            console.log(actualCapitalCity)
            resultTextElement.innerHTML = `aselfihaweflhib ${actualCapitalCity}`
